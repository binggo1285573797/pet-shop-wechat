...
Page({
  data: {
    ...
    stats: [
      { label: '新品数', value: '0+', iconType: 'new' },
      { label: '热门宠物', value: '0+', iconType: 'hot' }
    ],
    products: [],
    loading: false,
    currentBanner: 0
  ...
  loadData() {
    ...
    return api.getIndexData().then(res => {
      ...
      // 新品数（最近30天上架的商品数）
      const newProductCount = d.newProductCount || 0;
      // 热门宠物数（宠物类且销量>0的商品数）
      const hotPetCount = d.hotPetCount || 0;
      
      const stats = [
        { label: '新品数', value: newProductCount > 0 ? newProductCount + '+' : '0+', iconType: 'new' },
        { label: '热门宠物', value: hotPetCount > 0 ? hotPetCount + '+' : '0+', iconType: 'hot' }
      ];
      
      // 推荐商品（使用后端推荐算法：销量+上新时间+随机打散）
